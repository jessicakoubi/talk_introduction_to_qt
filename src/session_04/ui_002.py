...
if __name__ == "__main__":
    ...
    # Create a splash screen and brign it on top of other windows. For demo purpose on this file only we
    # set it on the first screen as the author of the demo love the multi-monitors life but want the splash-screen
    # to show in the screen shared on Zoom  during the session.
    screen = QtGui.QGuiApplication.screens()[0]
    splash = QtWidgets.QSplashScreen(screen, pixmap)
    ...
    # Added for demo purpose on this file only otherwise we don't see the splash-screen for long enough
    import time
    start = time.time()
    i = 1
    while time.time() - start < 4:
        ...
        app.processEvents()
        splash.showMessage(
            "Loading widget number {}".format(i),
            alignment=QtCore.Qt.AlignBottom | QtCore.Qt.AlignRight,
            color=QtGui.QColor("#fff"),
        )
        i+=1
